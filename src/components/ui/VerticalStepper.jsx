      ...
      <div className="hidden lg:flex flex-col fixed top-1/2 right-2 -translate-y-1/2 z-40 bg-white rounded-xl shadow border border-purple-200 p-2 gap-3">
        {steps.map((step, index) => {
          ...
          const circleClass = isActive
            ...
            : isClickable
            ? "bg-green-500 text-white border-green-500 hover:cursor-pointer hover:bg-green-600"
            : "bg-gray-200 text-gray-400 border-gray-300";

          return (
            <div key={index} className="flex flex-col items-center space-y-1">
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center border transition-all duration-200 ${circleClass}`}
                onClick={() => {
      ...
      <div className="lg:hidden fixed bottom-2 left-2 z-40 bg-white border shadow-md rounded-xl px-2 py-3 flex flex-col items-center gap-2 max-h-[90vh] overflow-y-auto">
        {steps.map((step, index) => {
          ...
          return (
            <div key={index} className="flex flex-col items-center space-y-1">
              <div
                className={`w-5 h-5 rounded-full flex items-center justify-center border transition-all duration-200 ${circleClass}`}
                onClick={() => {
                  if (isClickable) onStepClick(index);
                }}
              >
